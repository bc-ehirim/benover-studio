...
Build a production-quality static, one-page React + TypeScript + Tailwind content-generation app called “BENOVER Content Studio.” This must have no database, auth, user accounts, backend, API keys, or network calls. Use Framer Motion for restrained premium animations. It should feel like Apple + Linear + Notion: minimal, mobile-first, fast and polished. Light mode is the default, with a professional slate, blue, teal, and white palette. Use tasteful gradients/glows, local system typography, rounded 2xl panels, soft shadows, and clear visual hierarchy. Build actual functional client-side mock generation, copying, and txt downloading.
